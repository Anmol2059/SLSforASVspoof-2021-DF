...
# In train_epoch, pass domain labels
def train_epoch(train_loader, model, lr, optimizer, device):
    ...
    for batch_x, batch_y, domain_labels in tqdm(train_loader):  # changed part
        batch_size = batch_x.size(0)
        num_total += batch_size

        batch_x = batch_x.to(device)
        batch_y = batch_y.view(-1).type(torch.int64).to(device)
        domain_labels = domain_labels.to(device)  # changed part

        # Pass domain labels to the model
        batch_out = model(batch_x, domain_labels=domain_labels)  # changed part
